...
// NewRESPProtocol parses a RESP message and returns a RedisValue
// Reference: https://redis.io/docs/reference/protocol-spec
func NewRESPProtocol(byteStream *bufio.Reader) (Value, error) {
...
func decodeArray(byteStream *bufio.Reader) (Value, error) {
	...
	for i := 1; i <= count; i++ {
		value, err := NewRESPProtocol(byteStream)
		if err != nil {
			return Value{}, err
		}
